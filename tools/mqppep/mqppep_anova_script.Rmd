...
#
# get(v):x produces the leftmost element of v and removes it from v,
...
#
# put(v,x1,...,xn):v puts x1, x2, ..., xn onto the right end of v.
...
# This code adapted from matixcalc::is.positive.definite
  ...
  function(
    ...
    if (length(x) == 0L) {
      ...
      if (landscape)
        cat("\n\\begin{landscape}")
      if (!is.null(caption))
        if (use_subsubsection_header)
          subsubsection_header(caption)
        else
          subsection_header(caption)
      ...
      if (TRUE) {
        while (length(headings) > 0) {
          ...
          for (i in 1:(1 + length(tabstops))) {
            my_field <- get(headings)
            put(my_row, if (is.na(my_field)) "" else my_field)
          }
          latex_tabbing_row(
            v = my_row,
            underscore_whack = underscore_whack
...
subsection_header <-
  function(s) {
    ...
    cat(
      sprintf(
        "\\hypertarget{%s}{\\subsection{%s}\\label{%s}}\n",
        hyper, s, hyper
        )
      )
  }
...
subsubsection_header <-
  function(s) {
    ...
    cat(
      sprintf(
        "\\hypertarget{%s}{\\subsubsection{%s}\\label{%s}}\n",
        hyper, s, hyper
        )
      )
  }
...
low_fdr_barplot <- function(
  ...
  if (!is.null(rslt_score_list_i)) {
    ...
    if (nrow(k) > 0) {
      op <- par(
        fin = c(9, 2.0 + nrow(k) / 6),
        mar = par("mar") + c(0, 2.0, 0, 0)
      )
      on.exit(par(op))
  ...
  if (!is.null(rslt_score_list_i)) {
    ...
    if (write_db) {
      ...
      colnames(output_df) <-
        c(
          "Kinase",
          "\\(\\overline{\\log_2 (\\lvert\\text{fold-change}\\rvert)}\\)",
          "Enrichment",
          "Substrates",
          "z-score",
          "p-value",
...
hm2plus <- function(
  ...
  if (sum(!is.na(x)) < 1)
    return(NULL)
  nonax <- x
  nonax[is.na(x)] <- min(x, na.rm = TRUE)
...
draw_kseaapp_summary_heatmap <- function(
  ...
  ncol_x <- ncol(x)
  if (nrow_x < 2) {
    cat("No plot because matrix has ", nrow_x, " rows.\n\n")
    return(FALSE)
  } else if (ncol_x < 2) {
  ...
  if (print_trace_messages) cat(sprintf("my_row_cex_scale = %03f; ", my_row_cex_scale))
  if (print_trace_messages) cat(sprintf("my_col_cex_scale = %03f\n\n", my_col_cex_scale))
  if (print_trace_messages) cat(sprintf("my_row_cex_asterisk = %03f\n\n", my_row_cex_asterisk))
  if (print_trace_messages) cat(sprintf("my_row_cex = %03f; ", my_row_cex))
  if (print_trace_messages) cat(sprintf("my_col_cex = %03f\n\n", my_col_cex))
  if (print_trace_messages) cat(sprintf("my_row_cex = %03f; ", my_row_cex))
  if (print_trace_messages) cat(sprintf("my_col_cex = %03f\n\n", my_col_cex))
  hm2plus(
    x            = merged_scores,
...
trunc_ppep <- function(x) trunc_n(ppep_trunc_n)(x)
trunc_subgene <- function(x) trunc_n(subgene_trunc_n)(x)
trunc_enriched_substrate <- function(x) trunc_n(sbstr_trunc_n)(x)

# factory producing a function that returns
#   names of matrix rows where variance > n
var_gtr_n <- function(n) {
  function(x) {
    if (!is.matrix(x))
      return(NULL)
    keepers <- sapply(
  ...
  function(
    ...
    if (hm_heading_function(m, cutoff)) {
      ...
      if (nrow_m > 1) {
        ...
        col_margin <- master_cex * col_mchar_max * 1.5 # * (intensity_hm_rows - row_margin) / 16
        ...
        tryCatch(
          {
            ...
            my_row_cex <-
              master_cex * 200000 / (
                (max(nchar(rownames(m_margin)))^2) * intensity_hm_rows
                )
            ...
            my_col_cex <- master_cex * (my_row_cex ^ 8) * 0.0625 * row_margin / col_margin
            ...
            my_key_cex <- 0.9 - 0.1 * (intensity_hm_rows + nrow_m_hm) / intensity_hm_rows
            my_key_warp <- 1.5 * 22.75 / row_margin
            my_key_cex <- min(1.16, my_key_warp * my_key_cex)
            ...
            my_warp <- max(0.1, 1.4 * (6 + nrow_m) / intensity_hm_rows)
            # added 0.9 heuristically...
            my_plot_height <-
              (0.566 + 0.354 * (nrow_m / intensity_hm_rows)) *
                min(my_hgt_scale, my_hgt_scale * my_warp)
            my_plot_height <- min(3.65, my_plot_height * intensity_hm_rows / 50)
            my_heights <- c(
                0.3,                                 # title and top dendrogram
                my_plot_height,                      # plot and bottom margin
                4.14 - my_hgt_scale,                 # legend
                my_hgt_scale + 0.06 - my_plot_height # whitespace below legend
              )
            if (print_trace_messages) cat(sprintf("my_plot_height = %0.3f\n\n", my_plot_height))
            if (print_trace_messages) cat(sprintf("4.19 - my_hgt_scale = %0.3f\n\n", 4.19 - my_hgt_scale))
            if (print_trace_messages) cat(sprintf("nrow_m_hm = %0.3f; ", nrow_m_hm))
            if (print_trace_messages) cat(sprintf("ncol_m_hm = %0.3f; ", ncol_m_hm))
            if (print_trace_messages) cat(sprintf("my_row_cex = %0.3f; ", my_row_cex))
            if (print_trace_messages) cat(sprintf("my_col_cex = %0.3f; ", my_col_cex))
            if (print_trace_messages) cat(sprintf("my_key_cex = %0.3f\n\n", my_key_cex))
            if (print_trace_messages) cat(sprintf("my_hgt_scale = %0.3f; ", my_hgt_scale))
            if (print_trace_messages) cat(sprintf("my_plot_height = %0.3f; ", my_plot_height))
            if (print_trace_messages) cat(sprintf("my_warp = %0.3f\n\n", my_warp))
            if (print_trace_messages) cat(
              sprintf(
...
cov_heatmap <-
  function(
    ...
    ...                             # passthru to hm2plus or heatmap.2
      ...
      tmp <- m
      my_low_p_seq <- seq(
          from = min(intensity_hm_rows, nrow(m)),
          to = 1,
          by = -1
        )
      if (correlate_substrates) {
      ...
      my_corcov <- cov(t_m)
      prefiltered_nrow <- nrow(my_corcov)
      if (correlate_substrates && !is_positive_definite(my_corcov)) {
        my_correlate_substrates <- FALSE
        t_m <- t(m[my_low_p_seq, , drop = FALSE])
        t_m[is.na(t_m)] <- 0
        my_corcov <- cov(t_m)
        if (use_signed_sqrt) {
          my_xfrm <- var_gtr_1(my_corcov)
          my_corcov <- my_xfrm
        }
      } else if (correlate_substrates) {
        my_corcov <- cov2cor(my_corcov)
        my_correlate_substrates <- TRUE
      } else {
        my_correlate_substrates <- FALSE
        if (use_signed_sqrt) my_corcov <- var_gtr_1(my_corcov)
      }

      if (is.null(my_corcov)) {
        cat(
          sprintf(
            "\\newline\n%s %s plot.\n",
      ...
      if (sum(!is.na(t_m)) > 1)
        ...
        cat(
          sprintf(
            "\\newline\n%s %s plot.\n",
            "Not enough substrates had sufficient variance to produce",
            if (my_correlate_substrates)
              "correlation"
            else
              "covariance"
      ...
      my_margin <- 1 + my_row_cex * 64 / (8 + max_nchar)
      my_key_cex <- 1.4
      ...
      my_main <- sprintf("%s among %s substrates %s",
...
correlate_substrates <- params$correlateSubstrates
use_signed_sqrt <- params$useSignedSqrtCorr
ppep_trunc_n <- params$ppepTruncN
subgene_trunc_n <- params$subgeneTruncN
sbstr_trunc_n <- params$substTruncN
...
if (count_of_treatment_levels < 2) {
  ...
  for (cutoff in val_fdr) {
    ...
    if (number_of_peptides_found > intensity_hm_rows - 1) {
      cat("\\leavevmode\n\n\n")
      break
      }
...
$\text{FDR}_{j,i}$ is computed from the kinase enrichment score using the R `stats::p.adjust` function, applying the False Discovery Rate correction from Benjamini and Hochberg (1995) [doi:10.1111/j.2517-6161.1995.tb02031.x](https:/doi.org/10.1111/j.2517-6161.1995.tb02031.x)
...
For each enriched kinase, a heatmap showing the intensities is presented for up to 50 substrates, i.e., those substrates having the lowest p-values in ANOVA, using unimputed intensities where possible.
...
```{r ksea, echo = FALSE, fig.dim = c(12, 15), results = 'asis'}
...
```{r kseabar, echo = FALSE, fig.dim = c(9, 6), results = 'asis'}
cat("\n\\clearpage\n")
...
# was c(13, 14); c(12, 13.7) gives square aspect ratio for corcov plots
...
for (kinase_name in sort(enriched_kinases$kinase)) { # if (kinase_name == "PKCeta") {
  enriched_substrates <-
    all_enriched_substrates[
  ...
  if (nrow(m) > 0) {
    ...
    if (nrow(substrates_df) > intensity_hm_rows)
      substrates_df <- substrates_df[1:intensity_hm_rows, ]
    names(substrates_df) <- headers_2nd_line <-
      c("Substrate", "Sequence", "p-value")
    headers_1st_line <- c("", "", "ANOVA")
    if (1 < nrow(enriched_substrates))
      cat("\n\\newpage\n")
    subsubsection_header(sprintf("%s-substrate details", kinase_name))
    data_frame_tabbing_latex(
      x = substrates_df,
      tabstops = c(0.8, 5.2),
