...
params:
  ...
  kseaCutoffThreshold:  !r c(0.05, 0.1, 0.20, 0.25, 0.5, 0.9)[3]
  ...
  pdx_sampleGroupFilter:    !r c("none", "exclude", "include")[3]
  ...
  tst_sampleGroupFilter:    !r c("none", "exclude", "include")[1]
  ...
  sampleGroupFilterMode: !r c("r", "ri", "p", "pi", "f", "fi")[1]
...
<!--
  ...
  ut_sampleGroupFilter:    !r c("none", "exclude", "include")[1]
  ...
  groupFilterPatterns:  ".+,.*"
  sampleGroupFilter:    !r c("none", "exclude", "include")[1]
  imputationMethod:     !r c("group-median", "median", "mean", "random")[4]
  ...
  nd_sampleGroupFilter:    !r c("none", "exclude", "include")[1]
  ...
  pxd_sampleGroupFilter:    !r c("none", "exclude", "include")[3]
...
nbe <- function(..., f = cat, file = stderr()) {
  cat(
...
nbe("Please ignore the messages about systemd, if any.\n")
library(caret)
nbe("library(DBI)")
library(DBI)
nbe("library(RSQLite)")
...
see_kvp <-
  function(format, key, value, suffix = "") {
    ...
    ) stop("all arguments to see_kvp should be character")
...
see_variable <-
  function(x, suffix = "", digits = 3, xprssn = deparse1(substitute(x))) {
    if (is.character(x)) see_character(x, suffix, xprssn)
    else if (is.numeric(x)) see_numeric(x, suffix, digits, xprssn)
    else if (is.logical(x)) see_logical(x, suffix, xprssn)
    else {
      f <- file("")
      sink(f)
      str(x)
...
sample_group_filter <- params$sampleGroupFilter
if (grepl("f", params$sampleGroupFilterMode, fixed = TRUE)) {
  sample_group_filter_perl <- FALSE
  sample_group_filter_fixed <- TRUE
} else if (grepl("p", params$sampleGroupFilterMode, fixed = TRUE)) {
...
sample_group_filter_nocase <-
  grepl("i", params$sampleGroupFilterMode, fixed = TRUE)
...
ksea_min_kinase_count <- params$kseaMinSubstrateCount
...
if (is_string_null_or_empty(g_intensity_hm_criteria))
  stop("invalid intensityHeatmapCriteria parameter (must be string)")
switch(
  ...
  stop(
    sprintf(
      "invalid intensityHeatmapCriteria parameter %s (must be %s)"
    )
  )
)
...
if (!is.integer(g_intensity_hm_rows) || g_intensity_hm_rows < 1)
  stop("invalid intensityHeatmapRows (must be integer > 0")
...
if (!is.integer(g_intensity_min_per_class) || g_intensity_min_per_class < 0)
  stop("invalid intensityMinValuesPerGroup (must be integer > -1")

if (is.na(as.logical(g_correlate_substrates  <- params$correlateSubstrates)))
  stop("invalid correlateSubstrates (must be TRUE or FALSE)")

if (is.na(as.logical(g_filter_cov_var_gt_1   <- params$filterCovVarGT1)))
  stop("invalid filterCovVarGT1 parameter (must be TRUE or FALSE)")
...
# - maximum log2(FC)
# Produces data.frame with column names
...
ksea_scores <- function(
  ...
  nbe(see_variable(ksdata_dataset_abbrev))
  ...
  #ACE nbe(see_variable(new))
  #ACE ??? see_variable - str(new):
  #ACE 'data.frame':	13 obs. of  7 variables:
  #ACE  $ Protein    : chr  "Q9Y3B9" "Q6ZN18" "P63027" "P23763" ...
  #ACE  $ SUB_GENE   : chr  "RRP15" "AEBP2" "VAMP2" "VAMP1" ...
  #ACE  $ Peptide    : chr  "AAAAPDSRVSEEENLK" ...
  #ACE  $ SUB_MOD_RSD: chr  "AAAAPDSRVpSEEENLK" ...
  #ACE  $ p          : num  0.642 0.144 0.213 0.201 0.981 ...
  #ACE  $ FC         : num  1.177 0.111 1.646 3.181 1.014 ...
  #ACE  $ log2_fc    : num  0.2349 -3.1697 0.7191 1.6693 0.0205 ...
  ...
  nbe(see_variable(mean_log2_fc_j_all_i))
  ...
  mean_fc$enrichment <-
    mean_fc_m_s / abs(mean_log2_fc_j_all_i)
  #ACE HACK    mean_fc_m_s / mean(abs(log2_fc_j_each_i), na.rm = TRUE)
  ...
  nbe(see_variable(mean_fc[mean_fc$Kinase.Gene == "CK2alpha", ]))
  nbe(see_variable(mean_fc))
  #ACE # filter for m >= params$kseaMinSubstrateCount
  #ACE mean_fc <- mean_fc[mean_fc$m >= ksea_min_kinase_count, , drop = FALSE]
  #ACE #cat_variable(mean_fc)
...
# mar: The figure region dimensions, (width, height), in inches.
      ...
      function() {
        par_fin <- par("fin") # c(width_in_inches, height_in_inches)
        op <- par(
...
ksea_low_fdr_print <- function(
  ...
  if (!is.null(rslt_score_list_i)) {
    ...
    if (write_db) {
      ...
      nbe(see_variable(contrast_ksea_scores))
      ...
      output_order <- with(output_df, order(fdr))
...
hm2plus <- function(
  ...
  if (FALSE) # this is to avoid commenting out code to pass linting...
    my_hm2 <- show_invocation(heatmap.2, head_patch = FALSE)
  else
    my_hm2 <- heatmap.2
  ...
  if (sum(!is.na(x)) < 1)
    return(NULL)
  color_count <- 1 + max(64, length(as.vector(x)))
...
if (sample_group_filter %in% c("include", "exclude")) {
  ...
  if (sum(include_sample) < 2) {
    errmsg <- paste("ERROR: Too few samples for analysis;",
...
However, because as more and more phosphopeptides are tested, there is increasd probability that, by random chance, a given peptide will have a $p$-value that appears to indicte significance.  For this reason, the $p$-values were adjusted by applying the False Discovery Rate (FDR) correction from Benjamini and Hochberg (1995) [doi:10.1111/j.2517-6161.1995.tb02031.x](https:/doi.org/10.1111/j.2517-6161.1995.tb02031.x).
...
if (count_of_treatment_levels < 2) {
  ...
  for (cutoff in val_fdr) {
    ...
    bool_3 <- as.logical(as.integer(bool_1) * as.integer(bool_2))
    if (print_trace_messages) {
      if (length(bool_1) > 30) {
...
if (g_can_run_ksea && count_of_treatment_levels > 1) {
  ...
  nbe("CREATE TABLE contrast_lvl_lvl_metadata")
  dml_no_rows_exec(db, "
...
$$
\text{kinase enrichment }z{-score}_{j,i} = \frac{(\overline{s}_{j,i} - \overline{|p|}_j)\sqrt{m_{j,i}}}{\delta_j}
$$
...
- $\overline{{\lvert}p{\rvert}}_j$ is the mean $\text{absolute value}({\log_2 (\text{fold-change})})$ of all phosphosites identified in contrast $j$, and
  ...
  tryCatch(
    expr = {
        ...
        if (TRUE) {
          ksea_scores_rslt <-
            ksea_scores_rslt[
              ksea_scores_rslt$m >= ksea_min_kinase_count,
              ,
              drop = FALSE
            ]
        }
        ...
        if (FALSE) {
          nbe("Output contents of `ksea_scores_rslt` table\n")
...
if (have_kinase_descriptions) {
  ...
  # ACE all_enriched_substrates <-
  # ACE   all_enriched_substrates[
  # ACE     is.null(all_enriched_substrates$min_group_obs_count) || all_enriched_substrates$min_group_obs_count >= g_intensity_min_per_class,
  # ACE     ,
  # ACE     drop = FALSE
  # ACE   ]
  ...
  if (show_enriched_substrates && nrow(enriched_substrates) > 0) {
    ...
    nbe(see_variable(enriched_substrates))
    substrates_df <- with(
        enriched_substrates,
        data.frame(
          kinase = kinase,
    ...
    nbe(see_variable(substrates_df))
...
if (g_can_run_ksea) {
    ...
    if (nrow_m > 0) {
      ...
      substrates_df <- with(
          ...
          data.frame(
            ...
            anova_p_value = signif(fdr_adjusted_anova_p, 2),
            min_group_obs_count = signif(min_group_obs_count, 0),
            quality = signif(quality, 3) # round(quality)
          )
        )
  ...
  nbe("Output imputed, un-normalized data tabular file\n")
  write.table(
      data_table_imputed
    , file = imputed_data_filename
    , sep = "\t"
  ...
  data_table_imputed <-
      data_table_imputed[, c(1:12, 14:ncol(data_table_imputed))]
  nbe("Output quantile normalized data tabular file\n")
  write.table(
    data_table_imputed,
    file = imp_qn_lt_data_filenm,
    sep = "\t",
...
nbe("Output contents of `stats_metadata_v` table to tabular file\n")
write.table(
