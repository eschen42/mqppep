...
params:
  ...
  kseaAppPrepDb:        !r c(":memory:", "pST_Sites_NancyDu.ksea.sqlite")[2]
  run_ksea:             !r c(FALSE, TRUE)[2]
  show_toc:             true
...
latex_table_row <- function(v, extra = "") {
  ...
  function(
    ...
    if (length(x) == 0L) {
      ...
      if (!is.null(caption))
        cat("\\B \\\\ \\hline\\hline\n")
      # ref for top and bottom struts: https://tex.stackexchange.com/a/50355
      latex_table_row(v = colnames(m), extra = "\\T\\B")
      cat("\\hline\n")
...
ksea_scores <- function(
  ...
  # Save for now; TODO delete next line
  ksdata_dataset_disaggregated <- ksdata_dataset_abbrev
  ...
  mean_fc$z_score <- (
    (mean_fc$m_s - mean(new$log2_fc, na.rm = T)) *
  ...
  if (FALSE) {
    mean_fc$max_2_to_s <- 2 ** (max_fc[, 2] * sign(mean_fc$m_s))

    max_metadata <- sqldf(
      dbname = ":memory",
      x = "
        SELECT  a.rowid AS max_rowid,
                b.Protein,
                b.Gene,
                b.Peptide,
                b.FC,
                b.FC - a.max_2_to_s AS delta
          FROM
                mean_fc AS a,
                px AS b
          WHERE abs(b.FC - a.max_2_to_s) < 0.000001
          ORDER BY a.rowid
      "
      )
    mean_fc$max_prot <- max_metadata$Protein
    mean_fc$max_gene <- max_metadata$Gene
    mean_fc$max_pptd <- max_metadata$Peptide

    mean_fc$delta_fc <- max_metadata$delta

    mean_fc <- mean_fc[order(mean_fc$Kinase.Gene), -2]
    colnames(mean_fc) <- c(
      "Kinase.Gene", "mS", "Enrichment", "m", "z.score", "p.value", "FDR",
      "maxFC", "maxProtein", "maxGene", "maxPeptide", "deltaFC"
      )
  } else {
...
low_fdr_print <- function(
  ...
  if (!is.null(rslt_score_list_i)) {
    ...
    RSQLite::dbWriteTable(
      ...
      overwrite = db_write_table_overwrite,
...
# Helper function for ksea_heatmap
...
ksea_heatmap <- function(
  ...
  master <-
    Reduce(
      f = function(...) {
        base::merge(..., by = "Kinase.Gene", all = F)
      },
      x = score_list_m
      )
  ...
  function(
    ...
    if (hm_heading_function(m, cutoff)) {
      ...
      if (nrow(m) > 1) {
        ...
        #ACE margins <-
        #ACE   c(max(nchar(colnames(m_margin))) * 10 / 16, # col
        #ACE     max(nchar(rownames(m_margin))) *  7 / 16  # row
        #ACE   )
        #ACE margins <-
        #ACE   c(max(60, sqrt(nchar(colnames(m_margin)))) * 1 / 16, # col
        #ACE     max(60, sqrt(nchar(rownames(m_margin)))) * 7 / 16  # row
        #ACE   )
        #ACE }
        #ACE    max(80, sqrt(nchar(rownames(m_margin)))) * 7 / 16  # row
        margins <-
          c(0.5, # col
      ...
      try(
        if (nrow(m) > 1) {
          #char_contractor = 0.7 * 50 / max(100, nchar(rownames(m_margin)))
          char_contractor =
            250000 / (
...
ksea_heatmap_titles[[const_ksea_astrsk_kinases]] <-
  sprintf(
    "Summary figure for all contrasts and kinases with one or more %s < %s",
    ksea_cutoff_statistic,
    ksea_cutoff_threshold
    )
...
ksea_heatmap_titles[[const_ksea_nonastrsk_kinases]] <-
  sprintf(
    "Summary figure for all contrasts and kinases with no %s < %s",
    ksea_cutoff_statistic,
    ksea_cutoff_threshold
    )
    ...
    for (i in seq_len(length(levels(sample_treatment_levels)))) {
      ...
      if (ncol(lvlsbst) > 1) {
        quant_data_imp[ind, level_cols] <-
          apply(lvlsbst, 1, median, na.rm = T)[ind[, 1]]
      }
    }
...
#ACE quant_data_imp <- quant_data_imp[zero_sd_rownames,]
...
if (length(zero_sd_rownames) > 0 ) {
  ...
  zap_named_rows <- function(df, nms) { return (df[!(row.names(df) %in% nms),]) }
...
#  nrow(quant_data_imp_qn_log[is.na((apply(quant_data_imp_qn_log, 1, sd, na.rm = TRUE)) == 0),])

quant_data_imp_qn <- as.data.frame(quant_data_imp_qn)
#ACE not needed because of keep.names = TRUE above # names(quant_data_imp_qn) <- names(quant_data_imp)

write_debug_file(quant_data_imp_qn)

quant_data_imp_qn_log <- log10(quant_data_imp_qn)
#ACE not needed because of keep.names = TRUE above # rownames(quant_data_imp_qn_log) <- rownames(quant_data_imp)
...
#ACE not needed because of keep.names = TRUE above # rownames(quant_data_imp_qn_ls2) <- rownames(quant_data_imp)
...
#ACE not needed because of keep.names = TRUE above # rownames(quant_data_imp_qn_ls) <- rownames(quant_data_imp)
...
if (params$run_ksea && count_of_treatment_levels > 1) {
...
For each contrast two-way of treatments $j$, the score of each kinase $i$ is "normalized" (i.e., z-score transformed) using the formula:
...
Enrichment within contrast $j$ for kinase $i$ is computed as:
...
Color intensity in heatmaps reflects magnitude of $z$-score for enrichment of respective kinase in respective contrast.
...
if (params$run_ksea) {
  ...
  cat_enriched_heading <- function(m, cutoff) {
    cat("\\newpage\n")
    if (nrow(m) > intensity_hm_rows) {
      subsection_header(
        paste(
          sprintf("The %d enriched kinase-substrates with the lowest p-values,",
            intensity_hm_rows),
          sprintf( " KSEA %s < %0.2f\n", cutoff$statistic, cutoff$threshold)
        )
      )
    } else {
      if (nrow(m) == 1) {
        ...
        subsection_header(
          paste(
            sprintf("%d enriched kinase-substrates,", nrow(m)),
            sprintf( " KSEA %s < %0.2f\n", cutoff$statistic, cutoff$threshold)
          )
        )
      }
  ...
  enriched_substrates <- sqldf("
    SELECT ppep, '('||group_concat(gene||'-'||sub_gene)||') '||ppep AS label
      FROM (
  ...
  enriched_intensities <-
    as.matrix(unimputed_quant_data_log[enriched_substrates$ppep,])
  ...
  enriched_intensities <-
    enriched_intensities[na_counts < ncol(enriched_intensities) / 2,]
  ...
  if (nrow(m) > 0) {
    cut_args <-
      data.frame(
        statistic = ksea_cutoff_statistic,
        threshold = ksea_cutoff_threshold
      )
    number_of_peptides_found <-
      draw_intensity_heatmap(
        m                       = m,
