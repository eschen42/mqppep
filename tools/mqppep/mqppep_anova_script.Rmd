...
, "group-median" = {
    ...
    for (i in seq_len(length(levels(sample_treatment_levels)))) {
      ...
      if (ncol(lvlsbst) > 1) {
        the_centers <-
          apply(lvlsbst, 1, median, na.rm = T)
...
if (sum(is.na(quant_data)) > 0) {
  ...
  # ACE temp
  old <- quant_data_imp_log10
  # ACE is this obsolete?
  quant_data_imp_log10 <- log10(quant_data_imp)
  # ACE temp
  delta <- sum(old[!is.na(old)] - quant_data_imp_log10[!is.na(quant_data_imp_log10)])
...
if (run_ksea && count_of_treatment_levels > 1) {
